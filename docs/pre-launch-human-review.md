...
| **SAF-29 safety reporting** | `/report-concern` + transaction/footer entry points; server capture (logging + reporter privateData) | The 999/not-monitored copy; the **email-alert gap** (no mail transport yet → wire to safety@ once the provider lands); open-logged-out form (rate-limit?) | `server/api/safety-report.js`, `src/containers/SafetyReportPage/` |
